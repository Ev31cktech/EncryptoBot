...
namespace Bot.Utilities.Processed.Packet
{
	...
	public class Packet
	{
		...
		public Packet(rlbot.flat.GameTickPacket packet, EncryptoBot.EncryptoAgent bot)
		{
			...
			for (int i = 0; i < packet.BoostPadStatesLength; i++)
				bot.GetFieldInfo().BoostPads[i].BoostPad_Update(packet.BoostPadStates(i).Value);
